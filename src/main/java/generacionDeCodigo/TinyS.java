...
/**
 * La clase {@code TinyS} realiza el análisis sintáctico y léxico de un archivo de entrada en lenguaje TinyS.
 * <p>
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            if (parser.s()) {
                ...
                topVisitor = new TopVisitor(parser.getTablaSimbolos(), parser.getAST());
                topVisitor.generarCodigo();
                codigo = topVisitor.getCodigo();
                codigoGenerado = codigo.consolidar();

                resultadoAnalisis = "CORRECTO: GENERACION DE CODIGO\n";
            } else {
                // Este caso maneja una falla sin excepción, que podría indicar un parseo incompleto.
                resultadoAnalisis = "ERROR: GENERACION DE CODIGO\n";
            }
        ...
        // ────────────── Escritura de Codigo Generado ──────────────

        if (!codigoGenerado.isEmpty()) {
            try (BufferedWriter writer = new BufferedWriter(new FileWriter(nombreAsmSalida))) {
                writer.write(codigoGenerado);
            } catch (IOException e) {
                System.err.println("Error al escribir en el archivo de salida '" + nombreAsmSalida + "': " + e.getMessage());
            }
        }
