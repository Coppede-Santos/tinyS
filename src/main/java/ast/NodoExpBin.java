...
/** Clase que representa un nodo de expresión binaria en el AST */
public class NodoExpBin extends NodoExpUn{
    NodoExp ladoIzquierdo;

    /** Constructor de la clase NodoExpBin */
    ...
    public String chequeoDeSentencias(EntradaMetodo entradaMetodo, SymbolTable st, int profundidad) throws ErrorTiny {
        ...
        salida += tabs(profundidad + 1) + claveJson("tipoNodo")
                + valorJson("NodoExpBin") + ",\n";

        salida += tabs(profundidad + 1) +
                claveJson("ladoIzquierdo") + "{\n";
        salida += ladoIzquierdo.chequeoDeSentencias(
        ...
        salida += tabs(profundidad + 1) +
                claveJson("ladoDerecho") + "{\n";
        salida += ladoDerecho.chequeoDeSentencias(
                entradaMetodo, st, profundidad + 1
        );
        salida += tabs(profundidad + 1) + "},\n";

        if (ladoDerecho.getTipo() == null || ladoIzquierdo.getTipo() == null) {
        ...
        boolean algunTipoNoEsNumerico;
        algunTipoNoEsNumerico =
        ...
        if ((operador == TokenType.PLUS) || (operador == TokenType.MULT)){
            ...
            if ((!Objects.equals(tipoDer, "Int") &&
                    !Objects.equals(tipoDer, "Double") &&
                    !Objects.equals(tipoDer, "Str"))
                || (!Objects.equals(tipoIz, "Int") &&
                    !Objects.equals(tipoIz, "Double") &&
                    !Objects.equals(tipoIz, "Str"))){
                throw new ExpresionInvalidaError (
                        posicion, String.valueOf(operador)
                );
            }
            ...
            switch (tipoIz){
                ...
                case "Double":
                   if (!Objects.equals(tipoDer, "Double") &&
                           !Objects.equals(tipoDer, "Int")) {
                ...
                case "Str":
                    ...
                    tipo = "Str";
                    break;
                default: throw new ExpresionInvalidaError (
                        posicion,
        ...
        if(operador == TokenType.GREATER
                || operador == TokenType.GREATER_EQUAL
                || operador == TokenType.LESS
                || operador == TokenType.LESS_EQUAL){
            if (algunTipoNoEsNumerico) {
                throw new ExpresionInvalidaError (
                        posicion, String.valueOf(operador)
                );
        ...
        if (operador == TokenType.AND || operador == TokenType.OR){
            if (!Objects.equals(tipoDer, "Bool")
                    || !Objects.equals(tipoIz, "Bool"))
        ...
        if (encadenado != null) {
            salida += this.encadenado.chequeoDeSentencias(
                    entradaMetodo, st, tipo, profundidad + 1
            );
